...
> ⚠️ **本项目已停止维护（Archived）**
>
> HardCrop 的功能已整合进新项目 **[Opticon](https://github.com/IamCanincan/Opticon)**，后续维护与新功能都在那里进行。
> 本仓库保留仅供已安装旧版模块的用户参考，**不再发布新版本**。
>
> 仓库最新代码（未随版本发布）包含 Android 16 强制动态取色下未适配应用单色图标的遮罩反色修正；如需使用该能力请直接使用 Opticon。
...
Android 16 会把**没适配单色图标的应用**也强制主题化：桌面现场生成一张单色遮罩。
这张遮罩的极性是写死的（越亮越不透明），于是「深色字形 + 浅色底」的图标会被整体反掉——
字形变成洞，底色变成图形。

HardCrop 把桌面生成的这张遮罩**再翻一次**（`a → 255 - a`），让它回到正确的样子。

**已适配的应用不受影响**：桌面只在应用没提供 `monochrome` 图标（即
`AdaptiveIconDrawable.getMonochrome()` 为 null）时才会去生成遮罩，
所以"只动未适配的"是桌面自己的调用点保证的，模块不需要再判断一次。

改完要用 App 里的「清图标缓存并重启桌面」按钮重建一次才能看到效果。
